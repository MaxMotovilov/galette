...
function HKDF( salt, ikm ) {
	...
	for( var i=1; i<=n; ++i ) {
		...
		hmac.update( t );
		t = okm.slice( i*HASH_SIZE, (i+1)*HASH_SIZE + 1 );
...
module.exports = function( secret ) {
	...
	if( !secret )	secret = crypto.randomBytes( HASH_SIZE );
	...
	return function galette_KeyManager( id, cb ) {
		...
		var	ts = id ? parseInt( id, 16 ) : Date.now(), // May throw
			ikm = new Buffer( IKM_SIZE );

		ikm.writeUInt32LE( ts, 0, true );
		ikm.writeUInt16( ts >> 32, 4, true );
